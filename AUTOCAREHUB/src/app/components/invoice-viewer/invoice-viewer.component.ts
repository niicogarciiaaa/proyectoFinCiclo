...
export class InvoiceViewerComponent implements OnInit {
  ...
  generatePDF(): void {
    ...
    doc.text(
      `Número de factura: ${this.selectedInvoice.InvoiceID}`,
      marginLeft,
      topMargin + 10
    );
    doc.text(`Fecha: ${this.selectedInvoice.Date}`, marginLeft, topMargin + 20);
    doc.text(
      `Estado: ${this.selectedInvoice.Estado}`,
      marginLeft,
      topMargin + 30
    );
    ...
    doc.text(
      `Tel: ${this.selectedInvoice.WorkshopPhone}`,
      tallerX,
      tallerY + 30
    );
    ...
    doc.text(
      `Marca: ${this.selectedInvoice.Marca}`,
      marginLeft,
      topMargin + 60
    );
    doc.text(
      `Modelo: ${this.selectedInvoice.Modelo}`,
      marginLeft,
      topMargin + 70
    );
